...
def show_manipulability_ellipsoid(M):
    ...
    R = np.vstack(
        (np.hstack((eigenvectors, np.zeros((3, 1)))), np.array([0.0, 0.0, 0.0, 1.0]))
    )
...
if __name__ == "__main__":
    ...
    for i, target in enumerate(targets):
        ...
        start = perf_counter()
        q, rollout, success, message = ik.inverse(
        ...
        if success:
            print(
                "Solution found in {time:2.2f} seconds ({it} iterations).".format(
                    time=dt, it=len(rollout)
                )
            )
